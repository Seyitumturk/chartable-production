...
export default function DebugPage() {
  ...
  useEffect(() => {
    ...
    // Auto-fill clerk ID if user is authenticated
    if (user) {
      setClerkId(user.id);
    }
  }, [user]);
  ...
  const handleAddCredits = async () => {
    ...
    try {
      const response = await fetch('/api/stripe/test-credits', {
        method: 'POST',
        headers: {
    ...
    <main className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      <div className="container mx-auto px-4 py-8">
        ...
        <div className={`max-w-md mx-auto p-6 rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <h2 className="text-xl font-semibold mb-4">Add Credits Directly</h2>
          <p className="mb-4 text-sm opacity-70">
            This debug tool lets you add credits directly to an account by user ID or Clerk ID.
          </p>
          
          <div className="space-y-4">
            <div>
              <label className="block mb-2 text-sm font-medium">
                MongoDB User ID (optional)
              </label>
              <input
                type="text"
                value={userId}
                onChange={(e) => setUserId(e.target.value)}
                placeholder="67bd0e910d12ef861d7d33cd"
            ...
            <div>
              <label className="block mb-2 text-sm font-medium">
                Clerk ID (optional if User ID provided)
              </label>
              <input
                type="text"
                value={clerkId}
                onChange={(e) => setClerkId(e.target.value)}
                placeholder="user_2tVhirfU4nLRdCPEQ8sLVcVEDO9"
                className={`w-full p-2 rounded border ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'}`}
              />
            </div>
            ...
            <button
              ...
              disabled={loading || (!userId && !clerkId)}
              className={`w-full py-2 px-4 rounded font-medium ${
                loading || (!userId && !clerkId)
          ...
          {result && (
            <div className="mt-4 p-4 rounded border border-green-500/50 bg-green-500/20">
              ...
              <div className="text-sm space-y-1">
                ...
                <p><span className="opacity-70">Credits Added:</span> {result.user.creditsAdded.toLocaleString()}</p>
